var structureJS = (typeof structureJS != 'undefined') ? structureJS : {
  ...
  config : {
    core_base : '',/*This has to be set in script tag by user*/
    project_base : '', /*This is unknown to core build - only set in uer's project config*/
    manifest_name : 'core-manifest',/*Must be overridden by user*/
    config_name : 'config',
    global_base : '',
    globals : [],
    commons : []
  ...
  resolveFilePath : function(input){
    ...
    if( input && typeof input === 'object' )
      ...
      filePath = resolveDirectoryAliases(input, config.core_base);//config.core_base + input + '.js';
    ...
    core.loadConfigAndManifest(function(){
      ...
      core.loadScript(core.config.core_base + 'structureJS-dependency.js',function(){
        ...
        core_resolve.resolveDependencies(function(){
          ...
          /*User may override default config name 'config' in script tag*/
          core.config.config_name = structureJS.structureJSTag.getAttribute('data-config-name') || core.config.config_name;
          
          /*Project base is location of project config/manifest used to override 
          core build dir structure REQUIRED*/
